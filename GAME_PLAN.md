...
- 히어로 이동은 `NavigationAgent2D` + `NavigationRegion2D` 경로탐색 기반으로 처리한다.
- 내비게이션 이동 가능 영역은 `코어 중심 원형 플레이영역`이며, 코어 사각 충돌 영역은 내비게이션 홀(hole)로 제외한다.
- 이동 적용 순서는 아래를 따른다.
- 1) 우클릭 목표점을 플레이영역 경계로 1차 클램프
- 2) NavigationAgent2D로 다음 경로 포인트 계산
- 3) 경로 포인트 방향으로 `move_speed` 기반 스텝 이동
- 4) 목표 반경(`move_stop_distance`) 도달 시 정지/idle 복귀
- 기본 도착 허용 오차는 1px로 사용한다(`move_stop_distance=1.0`, `NavigationAgent2D path/target desired distance=1.0`).
- 경로 데이터가 유효하지 않으면 히어로는 즉시 정지하고 현재 위치를 유지한다.

### 이동 판정 규칙

- 현재 경로탐색 우회 대상은 정적 장애물(플레이영역 경계, 코어 사각 충돌)로 제한한다.

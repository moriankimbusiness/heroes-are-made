...
- 기본 플레이 영역 반경은 220이다.
- 코어 사각 충돌 영역은 히어로 이동 가능 영역에서 제외한다.
- 조작 방식은 좌클릭 선택 + 우클릭 이동 명령이다.
- 이동 명령은 선택된 히어로 1명에게만 적용한다.
- 이동 중에는 walk 애니메이션 상태를 유지한다.
- 이동 중에는 자동공격을 보류하고, 이동 종료 후 자동공격으로 복귀한다.
- 이동은 NavigationAgent2D + NavigationRegion2D 경로탐색 기반으로 처리한다.
- 이동 순서는 목표점 경계 클램프 → 경로 포인트 계산 → 스텝 이동 → 도착 판정 순서를 따른다.
- 도착 허용 오차(move_stop_distance)는 1px를 기준으로 운용한다.
- 경로 데이터가 유효하지 않으면 즉시 정지하고 현재 위치를 유지한다.

## 이동 판정 규칙
- 우회 대상은 정적 장애물(플레이영역 경계, 코어 사각 충돌)로 제한한다.
- 동적 장애물(적/다른 히어로) 회피는 현재 범위에서 제외한다.
- 경로 계산/스텝 적용은 이동 명령 활성 시에만 수행한다.

## 사거리 규칙
- 히어로 선택 시 실제 공격 사거리 원형을 표시한다.
- 사거리 계산식은 final_attack_range = max(1.0, (base_attack_range + attack_range_add) * attack_range_scale)다.
- 사거리 갱신은 초기화, 장비 변경, 스탯 재계산 시 즉시 반영한다.
- 향후 버프/디버프 연동 시에도 상태 변경 이벤트 기반 즉시 갱신을 유지한다.

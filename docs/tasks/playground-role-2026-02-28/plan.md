...
- [x] 사거리 오버레이 셀 계산 책임 분리 또는 인터페이스 명확화
...
## 5) 검증 체크리스트
- [x] 전투 시작 골드가 UI와 도메인 상태 모두 일치한다.
- [x] 카드 구매/리롤 후 골드 차감 및 버튼 상태가 정확히 반영된다.
- [x] 전투 종료 요약의 `gold_end`가 UI 경로가 아닌 도메인 경로로 수집된다.
- [x] `all_rounds_cleared`, `core_destroyed`, `all_heroes_dead` 종료 흐름 회귀가 없다.
- [x] 히어로 소환/이동/사거리 표시 동작이 리팩토링 전과 동일하다.

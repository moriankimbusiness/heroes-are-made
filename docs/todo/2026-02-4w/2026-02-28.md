...
- [x] 전투 종료 조건/리소스 해제 과업 마감: 승리/패배 3개 시나리오 + `enemy` 그룹 잔존 0 수동 검증 완료, `docs/tasks/battle-scene-end-condition-resource-cleanup-2026-02-28/result.md` 기록
...
- [x] 라운드 승리 기준 이관: `RoundSystem.final_round` Inspector 값을 `RoundManager`에 주입해 최종 라운드 판정을 데이터 길이와 분리
- [x] 스폰 데이터 구조 이관: `EnemySpawnController`의 `round_spawn_counts`/`health_multiplier`/공식 배율 제거, 레벨 JSON(`enemies`, 선택 `spawn_plan`) 로더 도입
- [x] 스폰 정책 확정 반영: `spawn_plan` 라운드 순서형 우선 + 미지정 라운드 `weight` 가중치 랜덤 fallback 구현(seed+node_id deterministic)
- [x] 레벨 오버라이드 정리: `level_02`의 라운드 관련 override 제거, `level_base` JSON 경로 기준으로 공통 테스트 경로 통일
...
- [x] `EnemySpawnController`는 라운드 시작 시 스폰 큐를 1회 구성하고 timeout 이벤트만 소비하도록 유지(신규 프레임 폴링 없음)
- [x] 가중치 선택은 캐시된 enemy 카탈로그를 사용하고, 런타임 리소스 로드는 JSON 초기화 시 1회만 수행
